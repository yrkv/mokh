...
See `Configuration` type alias for more details.
"""
...
def build_configuration(
    ...
    r"""Create a `Configuration` from a source dict, which is expected/intended
    to just be the content of a single config file.

    Note that this does NOT deep copy from the source dict. The source dict
    should be considered "used up" after building a configuration from it.
    """
    if out is None:
        out = {}
    ...
    for key, val in source.items():
        ...
        *the_rest, last = key.split('.')
        ...
        if isinstance(val, dict):
            ...
            build_configuration(val, out, prefix=next_prefix)
